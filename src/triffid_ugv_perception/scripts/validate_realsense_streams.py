...
def _decode_color_bgr(msg: Image):
...
def _decode_depth_u16(msg: Image):
...
def _save_debug_snapshots(save_dir: str, color_msg: Image, depth_msg: Image):
    os.makedirs(save_dir, exist_ok=True)

    color_bgr = _decode_color_bgr(color_msg)
    if color_bgr is not None:
        color_path = os.path.join(save_dir, 'color_debug.png')
        cv2.imwrite(color_path, color_bgr)
        print(f'  Saved color debug image: {color_path}')
    else:
        print('  Skipped color debug image: unsupported encoding')

    depth_u16 = _decode_depth_u16(depth_msg)
    if depth_u16 is not None:
        depth_raw_path = os.path.join(save_dir, 'depth_debug_raw.png')
        cv2.imwrite(depth_raw_path, depth_u16)
        print(f'  Saved depth raw image:   {depth_raw_path}')

        valid = depth_u16 > 0
        if np.any(valid):
            vmin = float(np.percentile(depth_u16[valid], 5))
            vmax = float(np.percentile(depth_u16[valid], 95))
            vmax = max(vmax, vmin + 1.0)
            depth_clipped = np.clip(depth_u16.astype(np.float32), vmin, vmax)
            depth_norm = ((depth_clipped - vmin) / (vmax - vmin) * 255.0).astype(np.uint8)
            depth_color = cv2.applyColorMap(depth_norm, cv2.COLORMAP_TURBO)
        else:
            depth_color = np.zeros((depth_u16.shape[0], depth_u16.shape[1], 3), dtype=np.uint8)

        depth_color_path = os.path.join(save_dir, 'depth_debug_color.png')
        cv2.imwrite(depth_color_path, depth_color)
        print(f'  Saved depth color image: {depth_color_path}')
    else:
        print('  Skipped depth debug image: unsupported encoding')
...
def main(args=None):
    ...
    parser.add_argument('--save-debug-dir', type=str, default='',
                        help='Directory to save color/depth debug snapshots')
    ...
    # Preferred mode is depth aligned to color. When resolutions differ,
    # ugv_node now runs a degraded fallback using scaled depth sampling.
    if node.color_image is not None and node.depth_image is not None:
        if (node.color_image.width, node.color_image.height) != (
            ...
            warnings.append(
                'Color/depth resolutions differ. Using legacy scaled-depth fallback '
                '(reduced geometric fidelity expected).'
            )
    ...
    if parsed.save_debug_dir and node.color_image is not None and node.depth_image is not None:
        print('\nDebug snapshots:')
        _save_debug_snapshots(parsed.save_debug_dir, node.color_image, node.depth_image)
